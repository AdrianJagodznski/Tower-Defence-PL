...
public plugin_init() 
{
	...
	iItem = td_shop_register_item("Zlote naboje - SNAJPERKI ", "Otrzymujesz 150 zlotych naboi do snajperek. Zadajesz 1.5x wiecej obrazen", 200, 0, id)
...
public info(id) {
	if(is_user_connected(id)) {
		...
		if(g_PlayerAmmo[id])
			client_print(id, print_chat, "[ZLOTE NABOJE] Posiadasz %d zlotych naboi do snajperek (NIE AWP)!", g_PlayerAmmo[id]);
...
public TraceAttackW(iEnt, iAttacker, Float:flDamage, Float:fDir[3], ptr, iDamageTaype)
{
	...
	client_print(iAttacker, print_center, "Zlote naboje - SNAJPERKI: %d", g_PlayerAmmo[iAttacker])
...
public TraceAttack(iEnt, iAttacker, Float:flDamage, Float:fDir[3], ptr, iDamageTaype)
{
	...
	client_print(iAttacker, print_center, "Zlote naboje - SNAJPERKA: %d", g_PlayerAmmo[iAttacker])
